...
public class UserController {
    ...
    public String createUser(@Valid @ModelAttribute CreateUserRequestDto user, HttpServletRequest request) {
    ...
    public String userProfile(Model model, HttpServletRequest httpServletRequest) {
        String currentPrincipalEmail = httpServletRequest.getUserPrincipal().getName();
        Optional<UserResponseDto> currentUser = userService.findByEmail(currentPrincipalEmail);
        String currentPrincipalName = currentUser.map(UserResponseDto::getName).orElse("No name provided");
        model.addAttribute("email", currentPrincipalEmail);
